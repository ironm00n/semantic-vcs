...
//! before, commits, records the op, and renders after (SPEC §4). Nothing here decides
...
pub struct Mutation {
    ...
    /// Set when a stale `open_changeset` row was closed on the way (SPEC §5.6).
...
impl Repo {
    ...
    /// parents. Identical content is a no-op (SPEC §2.5 step 3) and returns `cur`'s id.
