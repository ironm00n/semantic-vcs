...
use svc_core::engine::{add_def, edit_def, lookup_name, snapshot_files, rust_langs};
...
fn fixture() -> (MemStore, svc_core::Langs, svc_core::Snapshot) {
    ...
    files.insert(RelPath::new("src/lib.rs").unwrap(), b"fn ok() -> u32 { 1 }\n".to_vec());
...
fn edit_def_refuses_a_body_with_syntax_errors() {
    ...
    let err = edit_def(&store, &langs, &snap, ok, b"fn ok( -> u32 { let x = ; 1 }\n").unwrap_err();
...
fn add_def_refuses_a_body_with_syntax_errors() {
    let (store, langs, snap) = fixture();
    let err = add_def(&store, &langs, &snap, EntityId::new(), None, 1, b"fn broken( {}\n", Intent::Feature)
        .unwrap_err();
    assert!(matches!(err, Error::Parse(_)), "{err}");
}
...
fn ingest_of_a_broken_working_copy_still_succeeds() {
    ...
    let langs = rust_langs();
    let mut files = BTreeMap::new();
    files.insert(RelPath::new("src/lib.rs").unwrap(), b"fn ok() {}\nfn broken( {\n".to_vec());
    let snap = snapshot_files(&store, &langs, &files, None, ChangeId::new()).unwrap();
