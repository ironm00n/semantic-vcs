...
#[test]
fn rename_follows_use_super_mod_as_from_a_file_module() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let a = RelPath::new("src/a.rs").unwrap();
    ...
    assert!(
        text.contains("b::parse_file()"),
        "use super::a as b; b::parse from a file module must follow: {text}"
    );
}

#[test]
fn rename_follows_use_list_self_as_from_a_file_module() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let a = RelPath::new("src/a.rs").unwrap();
    ...
    let text = String::from_utf8(rendered.files[&foo].clone()).unwrap();
    assert!(
        text.contains("use crate::a::{self as b, parse_file};"),
        "self as b must keep the alias and follow parse: {text}"
    );
    assert!(text.contains("b::parse_file()"), "{text}");
    assert!(text.contains("parse_file();"), "{text}");
}

#[test]
fn rename_follows_pub_use_mod_as_then_crate_path() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let a = RelPath::new("src/a.rs").unwrap();
    ...
    files.insert(lib.clone(), b"mod a;\npub use a as b;\nmod foo;\n".to_vec());
    files.insert(a.clone(), b"fn parse() {}\n".to_vec());
    files.insert(foo.clone(), b"fn f() { crate::b::parse(); }\n".to_vec());
    let snap = snapshot_files(&store, &langs, &files, None, ChangeId::new()).unwrap();
    let a_parse = snap
        .entities
        .iter()
    ...
    let text = String::from_utf8(rendered.files[&foo].clone()).unwrap();
    assert!(
        text.contains("crate::b::parse_file()"),
        "pub use a as b; crate::b::parse must follow: {text}"
    );
}

#[test]
fn rename_follows_use_nested_mod_as() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let mut files = BTreeMap::new();
    files.insert(
        lib.clone(),
        b"mod a { pub mod inner { pub fn parse() {} } }\nuse a::inner as b;\nfn f() { b::parse(); }\n"
            .to_vec(),
    );
    let snap = snapshot_files(&store, &langs, &files, None, ChangeId::new()).unwrap();
    let id = snap
    ...
    assert!(
        text.contains("b::parse_file()"),
        "use a::inner as b; b::parse must follow: {text}"
    );
}

#[test]
fn rename_follows_enum_variant_path() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let mut files = BTreeMap::new();
    ...
    let text = String::from_utf8(rendered.files[&lib].clone()).unwrap();
    assert!(
        text.contains("enum ParseFile"),
        "enum def must follow: {text}"
    );
    assert!(
        text.contains("ParseFile::A"),
        "enum variant path must follow: {text}"
    );
}

#[test]
fn rename_follows_type_path_to_associated_const() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let mut files = BTreeMap::new();
    files.insert(
        lib.clone(),
        b"struct Parse;\nimpl Parse { const A: u8 = 1; }\nfn f() { let _ = Parse::A; }\n".to_vec(),
    );
    let snap = snapshot_files(&store, &langs, &files, None, ChangeId::new()).unwrap();
    let id = snap
        .entities
        .iter()
        .find(|(_, r)| r.name == "Parse" && r.kind == svc_core::Kind::Struct)
        .map(|(id, _)| *id)
        .expect("struct Parse");
    let next = rename(&store, &snap, id, "ParseFile").unwrap();
    let rendered = render(&next, &store, &langs, false).unwrap();
...
#[test]
fn rename_follows_use_inside_a_function() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let mut files = BTreeMap::new();
...
#[test]
fn rename_follows_raw_string_path_attr_inside_a_macro() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let bar = RelPath::new("src/bar.rs").unwrap();
    ...
    assert!(
        text.contains("use crate::foo::parse_file;"),
        "raw-string #[path] in a macro must attach: {text}"
    );
    assert!(
        text.contains("parse_file();"),
...
#[test]
fn rename_follows_macro_rules_inside_a_brace_body_macro_mod() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let caller = RelPath::new("src/caller.rs").unwrap();
    ...
    files.insert(
        lib,
        b"m! { pub mod fs { macro_rules! parse { () => {}; } } }\nmod caller;\n".to_vec(),
    );
    files.insert(
        caller.clone(),
...
#[test]
fn rename_follows_macro_export_macros_2_at_crate_root() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let caller = RelPath::new("src/caller.rs").unwrap();
...
#[test]
fn rename_follows_include_inside_a_mod() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    ...
    assert!(
        text.contains("crate::foo::parse_file()"),
        "include! inside a mod must attach that file's items: {text}"
    );
}

#[test]
fn rename_does_not_attach_include_inside_a_function() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let body = RelPath::new("src/foo_body.rs").unwrap();
    ...
    assert!(
        ...
        "include! inside a function must not attach to the mod: {text}"
    );
    assert!(
        !text.contains("parse_file()"),
        "include! in a fn must leave crate::foo::parse Free: {text}"
    );
}

#[test]
fn edit_def_follows_include_inside_a_mod() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let body = RelPath::new("src/foo_body.rs").unwrap();
    ...
    assert!(
        text.contains("crate::foo::parse_file()"),
        "edit-def must attach include! file items: {text}"
    );
}

#[test]
fn rename_follows_include_inside_a_soup_mod() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let body = RelPath::new("src/foo_body.rs").unwrap();
    ...
    assert!(
        text.contains("crate::foo::parse_file()"),
        "include! inside a soup mod must attach: {text}"
    );
}

#[test]
fn rename_follows_soup_impl_self_call() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let mut files = BTreeMap::new();
    ...
    let parse = snap
        .entities
        .iter()
        .find(|(_, r)| r.name == "parse" && r.kind == svc_core::Kind::Fn)
        .map(|(id, _)| *id)
        .expect("parse");
    let next = rename(&store, &snap, parse, "parse_file").unwrap();
    let rendered = render(&next, &store, &langs, false).unwrap();
    ...
    assert!(
        text.contains("Self::parse_file()"),
        "soup impl method body must bind Self::parse: {text}"
    );
}

#[test]
fn rename_follows_soup_impl_type_path() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let mut files = BTreeMap::new();
    ...
    let text = String::from_utf8(rendered.files[&lib].clone()).unwrap();
    assert!(
        text.contains("S::parse_file()"),
        "soup impl method body must bind S::parse: {text}"
    );
}

#[test]
fn rename_follows_soup_impl_self_field_call() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let mut files = BTreeMap::new();
...
#[test]
fn soup_impl_method_does_not_steal_crate_parse() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let mut files = BTreeMap::new();
    ...
    let parse = snap
        ...
        .iter()
        .find(|(_, r)| {
            r.name == "parse" && r.kind == svc_core::Kind::Fn && r.parent.is_none()
        })
        .map(|(id, _)| *id)
        .expect("crate parse");
    let next = rename(&store, &snap, parse, "parse_file").unwrap();
    ...
    assert!(
        text.contains("fn f() { parse_file(); }"),
        "soup impl method must stay out of the crate map: {text}"
    );
}

#[test]
fn rename_follows_soup_trait_self_call() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let mut files = BTreeMap::new();
...
#[test]
fn rename_follows_nested_mod_from_a_crate_root_include() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let a = RelPath::new("src/a.rs").unwrap();
    let inner = RelPath::new("src/outer/inner.rs").unwrap();
    let mut files = BTreeMap::new();
    files.insert(lib.clone(), b"include!(\"a.rs\");\nfn f() { crate::outer::inner::parse(); }\n".to_vec());
    files.insert(a, b"pub mod outer { pub mod inner; }\n".to_vec());
    files.insert(inner, b"pub fn parse() {}\n".to_vec());
    let snap = snapshot_files(&store, &langs, &files, None, ChangeId::new()).unwrap();
    let parse = snap
    ...
    assert!(
        text.contains("crate::outer::inner::parse_file()"),
        "nested mod in a crate-root include! must attach: {text}"
    );
}

#[test]
fn edit_def_follows_nested_mod_from_a_crate_root_include() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let a = RelPath::new("src/a.rs").unwrap();
    ...
    let (next, _) = edit_def(
        ...
        &snap,
        f,
        b"fn f() { crate::outer::inner::parse(); }\n",
    )
    .unwrap();
    let next = rename(&store, &next, parse, "parse_file").unwrap();
    let rendered = render(&next, &store, &langs, false).unwrap();
    ...
    assert!(
        text.contains("crate::outer::inner::parse_file()"),
        "edit-def must attach nested mod from a crate-root include!: {text}"
    );
}

#[test]
fn rename_follows_concat_include_at_crate_root() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let a = RelPath::new("src/a.rs").unwrap();
    ...
    assert!(
        text.contains("crate::outer::inner::parse_file()"),
        "include!(concat!(...)) at crate root must attach: {text}"
    );
}


#[test]
fn edit_def_follows_concat_include_at_crate_root() {
    let store = MemStore::new();
    let langs = rust_langs();
    let lib = RelPath::new("src/lib.rs").unwrap();
    let a = RelPath::new("src/a.rs").unwrap();
    let inner = RelPath::new("src/outer/inner.rs").unwrap();
    let mut files = BTreeMap::new();
    files.insert(lib.clone(), b"include!(concat!(\"a\", \".rs\"));\nfn f() {}\n".to_vec());
    files.insert(a, b"pub mod outer { pub mod inner; }\n".to_vec());
    files.insert(inner, b"pub fn parse() {}\n".to_vec());
    let snap = snapshot_files(&store, &langs, &files, None, ChangeId::new()).unwrap();
    let parse = snap
    ...
    assert!(
        text.contains("crate::outer::inner::parse_file()"),
        "edit-def must attach include!(concat!(...)): {text}"
    );
}
