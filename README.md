...
twelve hours of this repository's own history — 428 operations in 117 bundles —
were made through it, and the third command below opens them.

## Try it

```sh
nix develop            # or: a Rust toolchain plus jq, xz and node
cargo build -p svc
...
checkout, 28 MiB), checks it against `demo/history` (bundle list, op count,
...
the bundles in about 17 minutes. `--shell` instead of `--tui` gives a shell there.
...
- **History in operations.** `svc op log` is a journal of named operations —
  ...
  through revisions, `J`/`K` by pages, `e` entities, `o` the op log, `u` undo, `q` quit.)
...
| its own history replayed | 117 bundles, 428 operations, every tree as recorded, ≈17 min | `time demo/history/replay.sh <dir>` |
...
through `svc` verbs and are the 117 bundles in `demo/history/` (428
